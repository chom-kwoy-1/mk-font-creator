...
export function uniToPua(s: string): string {
    if (UNI_TO_PUA === null ||
        ...
        UNI_TO_PUA_3 = new Map(UNI_TO_PUA.entries().filter(a => a[0].length === 3));
        UNI_TO_PUA_2 = new Map(UNI_TO_PUA.entries().filter(a => a[0].length === 2));
        UNI_TO_PUA_1 = new Map(UNI_TO_PUA.entries().filter(a => a[0].length === 1));
    }
    s = s.normalize('NFKC');
